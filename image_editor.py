...
#global var to check the status of the flipped image
is_flipped = False
...
#function to draw on opened image
def draw(event):
    global file_path
    if file_path:
...
#function to erase lines over an image
def erase_lines():
    global file_path
    if file_path:
        canvas.delete("oval")
...
root = ttk.Window(title="MitBild", themename="superhero")
root.geometry("510x580+300+110")
root.resizable(0,0)
icon = ttk.PhotoImage(file='logo1.png')
...
#label within the frame
filter_label = ttk.Label(left_frame,text="Select Filter",background="black")
filter_label.pack(padx=0,pady=2)

#filters to add
filter = ["Contour","Black&White","Blur","Detail","Emboss","Edge Enhance","Sharpen","Smooth"]

#combobox for filters
filter_combobox = ttk.Combobox(left_frame, values=filter, width=15)
filter_combobox.pack(padx=10,pady=5)
filter_combobox.bind("<<ComboboxSelected>>",lambda event: apply_filter(filter_combobox.get()))

#loading icons for 4 buttons
image_icon = ttk.PhotoImage(file='image.png').subsample(10,10)
flip_icon = ttk.PhotoImage(file='flip.png').subsample(10,10)
rotate_icon = ttk.PhotoImage(file='rotate.png').subsample(10,10)
color_icon = ttk.PhotoImage(file='color.png').subsample(7,7)
erase_icon = ttk.PhotoImage(file='erase.png').subsample(10,10)
save_icon = ttk.PhotoImage(file='save.png').subsample(10,10)

#button to add/open the image file
